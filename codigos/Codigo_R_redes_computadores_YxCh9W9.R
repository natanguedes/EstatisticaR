...
dados1 = read.table("E:\\Natanael\\tsi\\3periodo\\estatistica\\municipiosbrasil.csv", header = T, sep = "\t") #o separador de tabulação é "\t" e da vírgula ";"
...
Amostra <- dados[sample(nrow(D2), 60),]
attach(Amostra)
dados = na.omit(Amostra)#remove null
dados
...
#Para salvar sua amostra, utilize o seguinte comando
 ...
 write.table(Amostra, "E:\\Natanael\\tsi\\3periodo\\estatistica\\minhaAmostraoficial2.txt", append = F, sep = "\t", row.names = F) #O comando append = F
...
Amostra <- read.table("E:\\Natanael\\tsi\\3periodo\\estatistica\\minhaAmostraoficial.txt", header = T, sep = "\t")
...
Idade = table(Idade)
hist(Idade)
hist(Idade, col = "blue","red","yellow","green")
...
plot(cor)
barplot(Cor, col = c("blue", "red",  "purple"))
barplot
